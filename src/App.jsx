...
function App() {
  ...
  function saveMediaPosition(mediaId, value) {
    if (!mediaId) return;
    ...
    positionSaveTimerRef.current = setTimeout(() => ipcRenderer.invoke('set-media-position', mediaId, seekPosition).catch(() => {}), 700);
  }
  ...
  return (
    <main className={`app-frame ${darkTheme ? 'app-frame--dark' : ''}`} onClick={() => { setContextMenu(null); setAppMenu(null); }}>
      ...
      <section className="workspace">
        {media && <MediaPlayer mediaId={media.id} src={media.playbackPath || `library/${media.media}`} captions={captions} hasCaptions={Boolean(media.caption)} volume={media.volume ?? 1} initialPosition={media.seekPosition ?? 0} autoplay={autoplayNext} onPlayingChange={setMediaWasPlaying} onCurrentPosition={(value) => { currentPositionRef.current = value; }} onPositionChange={saveMediaPosition} dark={darkTheme} showMedia={media.type !== 'audio'} fullscreenRequest={playerFullscreenRequest} videoFullscreenRequest={videoFullscreenRequest} transcribing={transcribingMediaId === media.id} queued={queuedTranscriptionIds.has(media.id)} transcriptionProgress={progress} onTranscribe={transcribeMedia} onVolumeChange={saveMediaVolume} onToggleTheme={() => setDarkTheme((value) => !value)} />}
      </section>
      </div>
